...
const Sidebar = () => {
  ...
  const employeeMenu = [
    { name: 'Dashboard', path: '/', icon: HomeIcon, color: 'from-blue-500 to-cyan-400' },
    { name: 'Profile', path: '/profile', icon: UserIcon, color: 'from-purple-500 to-pink-400' },
    { name: 'Attendance', path: '/attendance', icon: CalendarIcon, color: 'from-green-500 to-emerald-400' },
    { name: 'Apply Leave', path: '/leave', icon: DocumentTextIcon, color: 'from-amber-500 to-yellow-400' },
    { name: 'Payroll', path: '/payroll', icon: CurrencyDollarIcon, color: 'from-emerald-500 to-teal-400' },
    { name: 'Reports', path: '/reports', icon: ChartBarIcon, color: 'from-red-500 to-orange-400' },
  ];
  ...
  const adminMenu = [
    ...
    { name: 'Profile', path: '/profile', icon: UserIcon, color: 'from-purple-500 to-pink-400' },
    { name: 'Attendance', path: '/attendance', icon: CalendarIcon, color: 'from-green-500 to-emerald-400' },
    { name: 'Leave Approval', path: '/leave-approval', icon: DocumentTextIcon, color: 'from-amber-500 to-yellow-400' },
    { name: 'Payroll', path: '/payroll', icon: CurrencyDollarIcon, color: 'from-emerald-500 to-teal-400' },
    { name: 'Employees', path: '/employees', icon: UsersIcon, color: 'from-indigo-500 to-blue-400' },
    { name: 'Settings', path: '/settings', icon: CogIcon, color: 'from-gray-600 to-gray-400' },
  ];
              ...
              <div className="w-full bg-gradient-to-r from-indigo-500 to-purple-600 rounded-xl p-4 shadow-lg">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-purple-200 text-xs">{user?.role === 'admin' ? 'Admin Panel' : 'Employee Portal'}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {menuItems.map((item) => (
              <li key={item.name}>
                ...
                >
                  {({ isActive }) => (
                    <>
                      <div className={`relative ${(isMobile || !collapsed) ? 'mr-3' : ''}`}>
                        <item.icon className={`w-6 h-6 ${isActive ? 'text-white' : 'text-current'}`} />
                        <div className={`absolute -inset-1 bg-gradient-to-r ${item.color} rounded-lg blur opacity-0 group-hover:opacity-20 transition duration-300`}></div>
                      </div>
                      {(isMobile || !collapsed) && (
